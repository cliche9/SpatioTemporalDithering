...
def render_graph_SVAO():
    ...
    g.create_pass('GuardBand', 'GuardBand', {'guardBand': 0})
    ...
    g.create_pass('DiffuseTAA', 'TAA', {'alpha': 0.10000000149011612, 'colorBoxSigma': 0.5, 'antiFlicker': False})
    g.create_pass('AmbientTAA', 'TAA', {'alpha': 0.10000000149011612, 'colorBoxSigma': 1.0, 'antiFlicker': False})
    ...
    g.add_edge('DiffuseRef.out', 'SMAA.colorIn')
    ...
    g.add_edge('GBufferRaster.prevLinearZ', 'AmbientTAA.prevLinearZ')
    g.add_edge('LinearizeDepth.linearDepth', 'AmbientTAA.linearZ')
    g.add_edge('GBufferRaster.vbuffer', 'AmbientTAA.vbuffer')
    g.add_edge('GBufferRaster.vbuffer', 'DiffuseTAA.vbuffer')
    g.add_edge('SMAA.colorOut', 'DecimaTAA.fxaaIn')
    g.add_edge('DiffuseRef.out', 'DecimaTAA.colorIn')
    g.add_edge('GBufferRaster.mvec', 'DecimaTAA.mvec')
    ...
    g.mark_output('DecimaTAA.taaOut')
    return g
